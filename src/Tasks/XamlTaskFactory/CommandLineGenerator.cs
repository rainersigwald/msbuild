...
namespace Microsoft.Build.Tasks.Xaml
{
    ...
    public class CommandLineGenerator
    {
        ...
        {
            VerifyThrowArgumentNull(rule, nameof(rule));
            VerifyThrowArgumentNull(parameterValues, nameof(parameterValues));

            // Parse the Xaml file
            var parser = new TaskParser();
            bool success = parser.ParseXamlDocument(rule);
            VerifyThrow(success, "Unable to parse specified file or contents.");
        ...
        internal void GenerateCommandsAccordingToType(CommandLineBuilder clb, CommandLineToolSwitch commandLineToolSwitch, bool recursive)
        {
            ...
            switch (commandLineToolSwitch.Type)
            {
                ...
                default:
                    ...
                    VerifyThrow(false, "InternalError");
                    break;
            }
        }
            ...
            if (arguments != null)
            {
                foreach (Tuple<string, bool> arg in arguments)
                {
                    ...
                    else
                    {
                        if (arg.Item2)
                        {
                            throw new ArgumentException(FormatResourceStringStripCodeAndKeyword("Xaml.MissingRequiredArgument", parentSwitch, arg.Item1));
                        }
                    }
                }
            }
