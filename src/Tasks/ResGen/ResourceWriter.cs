...
namespace PassThroughResources
{
    ...
    public sealed class ResourceWriter : IResourceWriter
    {
        ...
        private bool _requiresNewReader = false;
        ...
        public void AddResourceData(string name, string typeName, byte[] serializedData, SerializationFormat format = SerializationFormat.BinaryFormatter)
        {
            if (name == null)
                throw new ArgumentNullException(nameof(name));
            if (typeName == null)
            ...
            _requiresNewReader |= format != SerializationFormat.BinaryFormatter;
 
            _preserializedData.Add(name, new PrecannedResource(typeName, serializedData, format));
        }
        ...
        private class PrecannedResource
        {
            ...
            internal PrecannedResource(string typeName, byte[] data, SerializationFormat format)
            {
                TypeName = typeName;
                Data = data;
                Format = format;
        ...
        public void Generate()
        {
            ...
            resMgrHeaderPart.Write(_requiresNewReader ? 
                typeof(ResourceSet).AssemblyQualifiedName :
                ResSetTypeName);
            ...
            using (dataSection)
            {
                ...
                foreach (var item in _resourceList)
                {
                    ...
                    {
                        if (_requiresNewReader && typeCode >= ResourceTypeCode.StartOfUserTypes)
                        {
                            data.Write((byte)userProvidedResource.Format);
                            Write7BitEncodedInt(data, userProvidedResource.Data.Length);
                        }
    ...
    internal enum ResourceTypeCode {
        ...
        // User types - serialized using SerializationFormat.
