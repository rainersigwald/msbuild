...
namespace Microsoft.Build.Tasks
{
    ...
    public class ResolveSDKReference : TaskExtension
    {
        ...
        private void LogResolution(SDKReference reference)
        {
            ...
            if (reference.Resolved)
            {
                ...
                if (reference.StatusMessages != null)
                {
                    ...
                    {
                        string formattedMessage = FormatResourceStringStripCodeAndKeyword(message.Item1, message.Item2);
        ...
        internal class SDKReference : IEquatable<SDKReference>
        {
            ...
            {
                VerifyThrowArgumentNull(taskItem, nameof(taskItem));
                VerifyThrowArgumentLength(sdkName, nameof(sdkName));
            ...
            private void GetSDKManifestAttributes()
            {
                if (_sdkManifest == null)
                {
                    ...
                    {
                        AddResolutionErrorOrWarning("ResolveSDKReference.ErrorResolvingSDK", ReferenceItem.ItemSpec, FormatResourceStringStripCodeAndKeyword("ResolveSDKReference.ErrorReadingManifest", _sdkManifestPath, _sdkManifest.ReadErrorMessage));
                    }
                }
