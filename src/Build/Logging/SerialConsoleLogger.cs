...
namespace Microsoft.Build.BackEnd.Logging
{
    internal class SerialConsoleLogger : BaseConsoleLogger
    {
        ...
        public override void TargetFinishedHandler(object sender, TargetFinishedEventArgs e)
        {
                  ...
                  || Verbosity == LoggerVerbosity.Diagnostic)
            {
                ...
                if (showTargetOutputs)
                {
                    ...
                    {
                        WriteLinePretty(GetResourceString("TargetOutputItemsHeader"));
                        foreach (ITaskItem item in targetOutputs)
                        {
                            WriteLinePretty(FormatResourceStringIgnoreCodeAndKeyword("TargetOutputItem", item.ItemSpec));
                        }
                    }
                }
        ...
        private string GetCurrentlyBuildingProjectFile()
        {
            ...
            {
                VerifyThrow(false, "Unexpected frame type.");
                return null;
            }
        }
            ...
            if (!contextStack.Peek().displayed)
            {
                ...
                switch (f.type)
                {
                    ...
                    case FrameType.Target:
                        ...
                        VerifyThrow(this.Verbosity < LoggerVerbosity.Detailed,
            ...
            )
        {
            ...
            {
                string errorMessage = "INTERNAL CONSOLE LOGGER ERROR. " + FormatString(unformattedMessage, args);
