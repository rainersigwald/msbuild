...
namespace Microsoft.Build.BackEnd
{
                                               ...
                                               INodePacket
    {
        ...
        {
            VerifyThrowArgumentNull(data, nameof(data));
            VerifyThrowInternalLength(data.ProjectFullPath, "data.ProjectFullPath");
        ...
        {
            VerifyThrowArgumentNull(instance, nameof(instance));
        ...
        {
            VerifyThrow(configId != InvalidConfigurationId, "Configuration ID must not be invalid when using this constructor.");
            VerifyThrowArgumentNull(other, nameof(other));
            VerifyThrow(other._transferredState == null, "Unexpected transferred state still set on other configuration.");
        ...
        public int ConfigurationId
        {
            ...
            set
            {
                VerifyThrow((_configId == InvalidConfigurationId) || (WasGeneratedByNode && (value > InvalidConfigurationId)), "Configuration ID must be invalid, or it must be less than invalid and the new config must be greater than invalid.  It was {0}, the new value was {1}.", _configId, value);
                _configId = value;
            }
        }
        ...
        public ProjectInstance Project
        {
            ...
            get
            {
                VerifyThrow(!IsCached, "We shouldn't be accessing the ProjectInstance when the configuration is cached.");
                return _project;
            }
            ...
            set
            {
                ...
                {
                    VerifyThrow(_transferredProperties == null, "Shouldn't be transferring entire state of ProjectInstance when transferredProperties is not null.");
        ...
        {
            VerifyThrow(project != null, "Cannot set null project.");
            _project = project;
            _baseLookup = null;
        ...
        internal void LoadProjectIntoConfiguration(
            ...
            int nodeId)
        {
            VerifyThrow(!IsLoaded, "Already loaded the project for this configuration id {0}.", ConfigurationId);
        ...
        private void InitializeProject(BuildParameters buildParameters, Func<ProjectInstance> loadProjectFromFile)
        {
            ...
            VerifyThrow(IsLoaded, $"This {nameof(BuildRequestConfiguration)} must be loaded at the end of this method");
        }
        ...
        public List<string> ProjectInitialTargets
        {
            ...
            set
            {
                VerifyThrow(_projectInitialTargets == null, "Initial targets cannot be reset once they have been set.");
        ...
        public List<string> ProjectDefaultTargets
        {
            ...
            set
            {
                VerifyThrow(_projectDefaultTargets == null, "Default targets cannot be reset once they have been set.");
        ...
        public Lookup BaseLookup
        {
            get
            {
                VerifyThrow(!IsCached, "Configuration is cached, we shouldn't be accessing the lookup.");
        ...
        {
            VerifyThrow(request.ConfigurationId == ConfigurationId, "Request does not match configuration.");
            VerifyThrow(_projectInitialTargets != null, "Initial targets have not been set.");
            VerifyThrow(_projectDefaultTargets != null, "Default targets have not been set.");
        ...
        public bool ShouldSkipIsolationConstraintsForReference(string referenceFullPath)
        {
            ...
            VerifyThrowInternalLength(referenceFullPath, nameof(referenceFullPath));
            VerifyThrow(Path.IsPathRooted(referenceFullPath), "Method does not treat path normalization cases");
        ...
        private ITranslator GetConfigurationTranslator(TranslationDirection direction)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    ThrowInvalidOperation("CacheFileInaccessible", cacheFile, e);
                }
