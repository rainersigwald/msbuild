...
namespace Microsoft.Build.Execution
{
    ...
    internal class LegacyThreadingData
    {
        ...
        internal RequestBuilder InstanceForMainThread
        {
            ...
            set
            {
                VerifyThrow(_instanceForMainThread == null || (_instanceForMainThread != null && value == null) || (_instanceForMainThread == value), "Should not assign to instanceForMainThread twice without cleaning it");
        ...
        internal void RegisterSubmissionForLegacyThread(int submissionId)
        {
            ...
            {
                VerifyThrow(!_legacyThreadingEventsById.ContainsKey(submissionId), "Submission {0} should not already be registered with LegacyThreadingData", submissionId);
        ...
        internal void UnregisterSubmissionForLegacyThread(int submissionId)
        {
            lock (_legacyThreadingEventsLock)
            {
                VerifyThrow(_legacyThreadingEventsById.ContainsKey(submissionId), "Submission {0} should have been previously registered with LegacyThreadingData", submissionId);
        ...
        internal WaitHandle GetStartRequestBuilderMainThreadEventForSubmission(int submissionId)
        {
            ...
            VerifyThrow(legacyThreadingEvents != null, "We're trying to wait on the legacy thread for submission {0}, but that submission has not been registered.", submissionId);
        ...
        internal Task GetLegacyThreadInactiveTask(int submissionId)
        {
            ...
            VerifyThrow(legacyThreadingEvents != null, "We're trying to track when the legacy thread for submission {0} goes inactive, but that submission has not been registered.", submissionId);
        ...
        internal void SignalLegacyThreadStart(RequestBuilder instance)
        {
            ...
            VerifyThrow(legacyThreadingEvents != null, "We're trying to signal that the legacy thread is ready for submission {0} to execute, but that submission has not been registered", submissionId);
