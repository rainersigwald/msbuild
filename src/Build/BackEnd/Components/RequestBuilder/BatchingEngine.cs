...
namespace Microsoft.Build.BackEnd
{
    ...
    internal static class BatchingEngine
    {
        ...
        )
        {
            ...
            {
                ThrowInternalError("Need the parameters of the batchable object to determine if it can be batched.");
            }

            if (lookup == null)
            {
                ThrowInternalError("Need to specify the lookup.");
            }
        ...
        {
            VerifyThrow(itemListsToBeBatched.Count > 0, "Need item types consumed by the batchable object.");
            VerifyThrow(consumedMetadataReferences.Count > 0, "Need item metadata consumed by the batchable object.");
