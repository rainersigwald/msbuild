...
namespace Microsoft.Build.BackEnd
{
    ...
    internal class AssemblyTaskFactory : ITaskFactory2
    {
        ...
        {
            ThrowInternalError("Use internal call to properly initialize the assembly task factory");
            return false;
        }
        ...
        {
            ThrowInternalError("Use internal call to properly create a task instance from the assembly task factory");
            return null;
        }
            ...
            bool isOutOfProc)
        {
            ...
            if (useTaskFactory)
            {
                VerifyThrowInternalNull(buildComponentHost, nameof(buildComponentHost));
        ...
        internal bool TaskNameCreatableByFactory(string taskName, IDictionary<string, string> taskIdentityParameters, string taskProjectFile, TargetLoggingContext targetLoggingContext, ElementLocation elementLocation)
        {
            ...
            try
            {
                VerifyThrowArgumentLength(taskName, "TaskName");
        ...
        private static IDictionary<string, string> MergeTaskFactoryParameterSets(IDictionary<string, string> factoryIdentityParameters, IDictionary<string, string> taskIdentityParameters)
        {
            ...
            else
            {
                ...
                {
                    ThrowInternalError("How did we get two runtime values that were unmergeable?");
                }
                else
                {
